...
function get_class_list() {
	...
	const file = fs.readFileSync("scene/register_scene_types.cpp", "utf8");

	const patterns = [/GDREGISTER_CLASS\((\w*)\)/, /register_class<(\w*)>/];
...
function get_icons() {
	...
	for (const file of fs.readdirSync("./editor/icons")) {
		if (extname(file) === ".svg") {
			let name = file;
	...
	// Modules
	for (const file of fs.readdirSync("./modules/csg/icons")) {
		if (extname(file) === ".svg") {
			let name = file;
			if (name.startsWith("icon_")) {
				name = name.replace("icon_", "");
